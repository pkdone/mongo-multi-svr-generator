...
${MONGO_SHELL_CMD} --quiet --port "${CONFIGS_PORT_PREFIX}0" --eval "var host='${HOST}', portPrefix='${CONFIGS_PORT_PREFIX}', numReplicas='${NUM_CONFIGS}'" js/initiate_csrs.js
printf "\n"
...
for shard in `seq 0 ${MAX_SHARD}`; do
    ...
    ${MONGO_SHELL_CMD} --quiet --port "${SHARDS_PORT_PREFIX}${shard}0" --eval "var host='${HOST}', portPrefix='${SHARDS_PORT_PREFIX}', shard=${shard}, numReplicas='${NUM_REPLICAS_PER_SHARD}'" js/initiate_replicaset.js
    printf "\n"
done
...
for router in `seq 0 ${MAX_ROUTER}`; do
    ...
    printf "printf \"Router mongos server ${router}: \"\n" >> ${OUTFILE}
...
${MONGO_SHELL_CMD} --quiet --host "${HOST}" --port "${ROUTERS_PORT_PREFIX}0" --eval "var shardReplicaSetsURI='${shardReplicaSetsURIList}', dbname='${DB_TO_SHARD}', colctnname='${COLCTN_TO_SHARD}', shardkey='${SHARD_KEY}'" js/configure_shards.js
printf "\n"
